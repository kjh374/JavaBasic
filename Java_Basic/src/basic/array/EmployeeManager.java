		...
		while(true) {
			...
			if(menu == 1) {
				...
				if(count==0) {
					...
					for(int i=0; i<count; i++) {
						System.out.printf("%s  %s  %d  %s"+userNums[i], names[i], ages[i], departments[i]);
					}
				}
				System.out.println("나가시려면 아무 키나 누르세요.");
				System.out.print("> ");
...
//						flag = true;
				...
				for(int i=0; i<count; i++) {
					if(userNums[i].equals(empNum)) {
						...
						menu4: while(true) {
							System.out.println("[1. 나이변경 | 2. 부서 변경 | 3. 취소]");
							System.out.println("> ");
							int sel = sc.nextInt();
							switch(sel) {
							case 1:
							...
							case 2:
								...
								break;
							case 3:
								System.out.println("정보 수정을 취소하고 메뉴로 나갑니다.");
								break menu4;
							default:
