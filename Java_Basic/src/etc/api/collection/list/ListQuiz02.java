...
//		User u1 = new User("신형만", 30);
//		User u2 = new User("봉미선", 29);
//		User u3 = new User("신짱아", 3);
//		
//		Collections.addAll(user, u1, u2, u3);
		
//		user.add(new User("신형만", 30));
//		user.add(new User("봉미선", 29));
//		user.add(new User("신짱아", 3));
		
		Collections.addAll(user, new User("신형만", 29), new User("봉미선", 28), new User("신짱아", 3));
...
//		}
		...
		for(int i=0; i<user.size(); i++) {
			if(user.get(i).getName().equals("신형만")) {
				user.remove(i);
			}
		}
		System.out.println("삭제 후 정보: " + user);
	}
}
